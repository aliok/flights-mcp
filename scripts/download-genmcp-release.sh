...
LATEST_VERSION=$(curl -s https://api.github.com/repos/genmcp/gen-mcp/releases/latest | grep '"tag_name":' | sed -E 's/.*"tag_name": "([^"]+)".*/\1/')
...
DOWNLOAD_URL="https://github.com/genmcp/gen-mcp/releases/download/${LATEST_VERSION}/${ZIP_FILE}"
...
echo "gen-mcp downloaded successfully to: $GENMCP_DIR/${BINARY_NAME}"
